...
| `interaction.fixture.ts` | chat-input / autocomplete / context-menu / help / select fakes with a `calls` log; each takes the `locale` its handler reads, plus `userId`/`guildId` (`USER_ID`/`GUILD_ID` by default, `guildId: null` for a DM). `lastReplyDescription()` reads the ephemeral answer, `lastPostDescription()` / `lastPostPayload()` what landed in the channel (`send`, or the `reply` the message triggers post), `lastPostFlags()` whether it was silenced. `inThread` on every surface's options makes the post target a thread. `makeSendTyping()` records `sendTyping` into the same log and every trigger's channel carries it, so `typingCount()` counts the calls and `typedBeforePosting()` asserts the channel was shown the bot working before the translation landed |
| `post.fixture.ts` | `CHANNEL_ID` / `POSTED_MESSAGE_ID`, `makePostRef()`, and `makePostedMessages()` — a fresh id per post, so a fixture driven twice records two distinct posts and the registry keeps both; `makeTypingChannel()` is a bare `PostChannel` for `typing.test.ts`, counting calls in its own `typingCount` field, with `withoutTyping` omitting `sendTyping` and `failing` making it reject |
...
| `message.fixture.ts` | `makeMentionMessage()` for the mention trigger; its `author` is a recipient, so `message.author.dms` holds the refusals it was sent (`dmsClosed` makes them fail); `typingFails` makes its channel refuse `sendTyping`, the one trigger that covers that path |
